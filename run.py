...
if __name__ == '__main__':
    ...
    prefix_splits = SplitIID(split_config=iid_split_config).split()
    logger.info("Splitting non-IID dataset")
    prefix_splits += SplitNonIID(split_config=non_iid_split_config).split()
    for local_prefix in prefix_splits:
        ...
        tag = '_'.join([local_prefix_qc.split('/')[i] for i in [-3, -1]])
