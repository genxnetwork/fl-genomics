...
def read_local_local(exp_name, selected_metrics):
...
def read_local_global(exp_name, selected_metrics):
...
def read_centr(exp_name, selected_metrics):
...
def impute_missing_columns(df_local_globaltestset, df_feder, df_local_localtestset, df_centr):
    ...
    return df_local_globaltestset, df_feder


def prepare_data_to_plot(df_local_localtestset, df_local_globaltestset, df_centr, df_feder, metric_name):
    # concat datasets
...
def plot(df, df_local_localtestset, out_fn):
...
def continuous():
    ...
    df_local_localtestset = read_local_local(exp_name='local-models-local-snps', selected_metrics=SELECTED_METRICS)

    # reading local-global: local model tested on global test set
    df_local_globaltestset = read_local_global(exp_name='local-models-local-snps-centralized-metrics', selected_metrics=SELECTED_METRICS)

    # reading centralized model tested on global test set
    df_centr = read_centr(exp_name='centralized-models-centralized-snps-fixed', selected_metrics=SELECTED_METRICS)

    # reading federated model tested on global test set
    df_feder = read_feder(exp_name='fl-ukb-mg-avg', selected_metrics=SELECTED_METRICS)

    # imputing missing tags
    df_local_globaltestset, df_feder = impute_missing_columns(df_local_globaltestset=df_local_globaltestset,
                                                              df_feder=df_feder,
    ...
    df = prepare_data_to_plot(df_local_localtestset=df_local_localtestset,
                              ...
                              df_centr=df_centr,
                              df_feder=df_feder,
                              metric_name='metrics.val_r2')

    plot(df, df_local_localtestset=df_local_localtestset, out_fn='/home/genxadmin/tmp.html')


def binary():
    # reading local-local: local model tested on local (native) test set
    df_local_localtestset = read_local_local(exp_name='categorical-local-models-local-snps',
                                             selected_metrics=['metrics.train_roc_auc', 'metrics.val_roc_auc'])

    # reading local-global: local model tested on global test set
    df_local_globaltestset = read_local_global(exp_name='categorical-local-models-local-snps-centralized-metrics',
                                             selected_metrics=['metrics.train_roc_auc', 'metrics.val_roc_auc'])

    # reading centralized model tested on global test set
    df_centr = read_centr(exp_name='categorical-centralized-models-centralized-snps',
                                             selected_metrics=['metrics.train_roc_auc', 'metrics.val_roc_auc'])

    # reading federated model tested on global test set
    df_feder = read_feder(exp_name='fl-ukb-mg-avg-bin',
                                             selected_metrics=['metrics.train_auc', 'metrics.val_auc'])
    df_feder['metric_name'] = df_feder['metric_name'].replace({'metrics.train_auc': 'metrics.train_roc_auc',
                                                               'metrics.val_auc': 'metrics.val_roc_auc'})
    ...
    df = prepare_data_to_plot(df_local_localtestset=df_local_localtestset,
                              ...
                              df_centr=df_centr,
                              df_feder=df_feder,
                              metric_name='metrics.val_roc_auc')

    plot(df, df_local_localtestset=df_local_localtestset, out_fn='/home/genxadmin/tmp_bin.html')
