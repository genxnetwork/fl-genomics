...
class SplitTG(SplitBase):
    ...
    def get_ethnic_background(self, min_samples_in_pop = 30) -> pd.DataFrame:
        """
        ...
        """
        y = pd.read_csv(os.path.join(TG_DATA_ROOT, 'igsr_samples.tsv'), sep='\t').rename(columns={'Sample name': 'IID', 'Population code': 'pop', 'Superpopulation code': 'split'})
        ...
        pop_val_counts = y['pop'].value_counts()
        y = y[y['pop'].isin(pop_val_counts[pop_val_counts >= min_samples_in_pop].index)]
        return y[['IID', 'pop', 'split', 'Population name', 'Superpopulation name']]
