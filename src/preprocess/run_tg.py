...
if __name__ == '__main__':
    ...
    # 4. Perform pruning for each node separately
        ...
        nodes_noall = nodes[:]
        nodes_noall.remove('ALL')
        pruning.merge(node='ALL_filtered', nodes_list=nodes_noall)
        for node in nodes:
            logger.info(f'Removing variants from {node}')
            pruning.remove_variants(NODE=f'{node}_filtered', SNPS='ALL_filtered')
        ...
        for node in nodes:
            splitter.split_ids(ids_path=os.path.join(SPLIT_GENO_DIR, f'{node}_filtered.pruned.psam'), node=node,
                               y=ancestry_df, random_state=0)
            ...
            for fold_index in range(FOLDS_NUMBER):
                for part_name in ['train', 'val', 'test']:
                    ...
                    ancestry_df.loc[relevant_ids, ['IID', 'ancestry']].to_csv(
                        superpop_split.get_phenotype_path(node=node, fold_index=fold_index, part=part_name), sep='\t',
                        index=False)
            ...
            for node in ['ALL']:
                for part_name in ['train', 'val', 'test']:
                    run_plink(
                        args_list=[
                            '--pfile',
                            superpop_split.get_pfile_path(node=node, fold_index=fold_index, part_name=part_name),
                            '--read-freq',
                            superpop_split.get_pca_path(node='ALL', fold_index=fold_index, part='train', ext='.acount'),
                            '--score', superpop_split.get_pca_path(node='ALL', fold_index=fold_index, part='train',
                                                                   ext='.eigenvec.allele'), '2', '5', 'header-read',
                            'no-mean-imputation', 'variance-standardize', '--score-col-nums', '6-25',
